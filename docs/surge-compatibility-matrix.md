...
| rurge 专有 | 守护进程 | `rurge run -c <path> [--tun] [--system-proxy]`、`rurge service install/uninstall`、`rurge mitm ca generate/export` | 1 / 3 / 4 | `--system-proxy`（环境变量 `RURGE_SYSTEM_PROXY`）与 `rurge service install -c <conf> [--user] [--system-proxy] [--dry-run]` / `rurge service uninstall [--user] [--dry-run]` 已实现（M4b；`uninstall` 没有 `-c` 也没有 `--system-proxy`）；Windows 除 Ctrl-C 外，控制台关闭、注销、系统关机也会先恢复系统代理再退出；Windows 开机自启用登录时触发的计划任务（`schtasks /sc onlogon /f`，🟡，真正的 Windows 服务留到阶段 6），`--user` 在 Windows 上无实际区别（`schtasks` 不分用户 / 系统范围） |
