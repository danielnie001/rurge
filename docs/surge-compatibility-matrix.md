...
| `exclude-simple-hostnames` | 布尔；默认 false | 全部 | 🟡 | 1 | Windows 写入 `ProxyOverride` 的 `<local>`（✅）；macOS 经 `networksetup` 没有对应的命令行选项，无法设置，`true` 时打一条 WARN 并忽略这一项，其余系统代理设置照常应用（阶段 6 评估改用 SystemConfiguration 框架直写）；Linux GNOME / KDE 无对应项，该选项在 Linux 上不起作用（`gnome_apply_commands` / `kde_apply_commands` 都不读取它，不像 macOS 那样打 WARN，无日志） |
...
| `internet-test-url` | URL；默认 `http://bing.com/` | 全部 | ✅ | 2 | M3b 起是直连类策略（`DIRECT` 与 `direct` 别名）连通性测试的默认 URL（策略自己的 `test-url` 优先）；取值不是 http:// 或 https:// URL 时 `W0012` 告警（不引用取值）并沿用默认值 |
| `proxy-test-url` | URL；默认 `http://bing.com/` | 全部 | ✅ | 2 | M3b 起是代理策略连通性测试的默认 URL（策略自己的 `test-url` 优先）；取值不是 http:// 或 https:// URL 时 `W0012` 告警（不引用取值）并沿用默认值 |
...
| `shadow-tls-sni` | 主机名；v3 必填 | ✅ | 2 | M2c 已实现：必须是 DNS 名（IDN 写成 `xn--` 形式；IP 字面量 `E0018`）；v3 缺它 `E0018`。v2 不写时**不发 SNI**（手册），证书按策略自己的 TLS 会用的名字校验——`sni` 写了名字用它，否则用服务器主机名；所以服务器写成 IP、伪装站点是别人的网站时必须写它，否则握手以 `shadow-tls: the camouflage handshake failed: …` 失败。`server-cert-verify-name` 刻意不参与这次校验——它命名的是代理自己的证书，不是伪装站点的证书 |
...
| `GET/POST /v1/features/system_proxy` | 系统代理开关 | Mac only | 🟡 | 1 | M4b 已生效：`GET` 返回真实状态（`Control::system_proxy_enabled`）；`POST` 走 `Control::set_system_proxy`，成功 `{}`，失败 500 `{"error":"<原因>"}`。标 🟡 的原因：Windows 只写 / 备份 `ProxyEnable`、`ProxyServer`、`ProxyOverride` 三个值，不清除也不保存 `AutoConfigURL`（PAC）——系统配置了 PAC 时 Windows 可能继续优先用 PAC，rurge 写的手动代理不生效；某个值类型不对（如 `ProxyEnable` 不是 DWORD）也会让 `snapshot` 报错退出。macOS 经 `networksetup` 逐个启用的网络服务设置，是否需要管理员账户未在真机验证（设计文档 Q2），标准账户下的拒绝文案由 rurge 原样传递；`exclude-simple-hostnames` 在 macOS 上的限制见 2.1 节该行。Linux 仅 GNOME（`gsettings`）与 KDE（`kwriteconfig6`/`5`）两种桌面下生效，其它桌面不改动任何设置，`apply` 失败并给出 `export http_proxy=… https_proxy=… no_proxy=…` 提示（对应 `rurge run --system-proxy` 退出 1、这个端点返回 500）。另有一条**未验证**的疑点：Windows 的 `ProxyServer` 里 `socks=<addr>` 这一段，基于 WinINet 的客户端历史上按 SOCKS4 解释，而 rurge 的入站只讲 SOCKS5——浏览器与系统组件是否真的用得上这一段没有在真机确认过，已列入 `docs/acceptance/phase1-manual.md` 的手工验收清单 |
