...
| 终审 A | 6.5："`dial_internal`（DNS 跟随出站模式）不需要改动：代理出站对它同样可用" | 一旦代理是真的（能力表翻转之后），这句就错了：`encrypted-dns-follow-outbound-mode` 的防环回退（FR-DNS-04 / M3 设计 §7.4）从未实现，DNS 会话命中以域名配置的代理时，连这个代理又需要正在等待的这次查询，解析会一直挂到超时。整分支终审时补上（`crates/rurge-engine/src/engine.rs`）：新增 `socket_opener`，沿 `underlying-proxy`（经组的当前成员）找到真正打开 socket 的那一跳，走查深度以 `rurge_policy::registry::MAX_DEPTH` 为界；这一跳的 `server` 是域名就告警（只报策略名，不报服务器主机名）、把会话标成 `dns-follow: proxy configured by host name bypassed to avoid a resolution loop` 并经 `fallback` 直连。以 IP 配置的代理没有环，照常承载 DNS 会话。兼容性清单里两条 `encrypted-dns-follow-outbound-mode` 的记录（2.1 / 6.2）此前就写着有这条防环逻辑，至此才名副其实 |
