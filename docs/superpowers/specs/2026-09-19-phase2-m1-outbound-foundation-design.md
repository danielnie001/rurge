...
| 终审 A | 6.5："`dial_internal`（DNS 跟随出站模式）不需要改动：代理出站对它同样可用" | 一旦代理是真的（能力表翻转之后），这句就错了：`encrypted-dns-follow-outbound-mode` 的防环回退（FR-DNS-04 / M3 设计 §7.4）从未实现，DNS 会话命中以域名配置的代理时，连这个代理又需要正在等待的这次查询，解析会一直挂到超时。整分支终审时补上（`ae6bbf5`，`crates/rurge-engine/src/engine.rs`）：新增 `socket_opener`，沿 `underlying-proxy`（经组的当前成员）找到真正打开 socket 的那一跳，走查深度以 `rurge_policy::registry::MAX_DEPTH` 为界；这一跳的 `server` 是域名就告警（只报策略名，不报服务器主机名）、把会话标成 `dns-follow: proxy configured by host name bypassed to avoid a resolution loop` 并经 `fallback` 直连。以 IP 配置的代理没有环，照常承载 DNS 会话。**复审残留（9a5bba2）**：初版的 `socket_opener` 在下一跳的终端不是 Proxy 时一律返回 `None`，漏掉了底下是 DIRECT 的情形（`direct` 别名策略，或当前选中 DIRECT 的组）：那时打开 socket 的是 DIRECT，而它要在本机解析的正是上面那一跳的服务器名，同样成环；现改为对 `TerminalKind` 逐项匹配（Direct → 返回当前这一跳）。兼容性清单里两条 `encrypted-dns-follow-outbound-mode` 的记录（2.1 / 6.2）此前就写着有这条防环逻辑，至此才名副其实 |
| 终审 C | 6.5（绝对 URI 转发）未说明上游对转发的请求回错误时怎么办 | 转发模式下上游回 407 记为失败会话并回 502 页面：这是 rurge 自己给上游的凭据不对，客户端无从处理，而 `Proxy-Authenticate` 是逐跳头，剥掉之后交还给客户端的是一个非法的 407。两段文本都只由状态码构成，不取上游的原因短语；上游的其它响应（含 502）原样交还客户端。**连带的结构改动**（`crates/rurge-inbound/src/http.rs::forward`）：连接驱动任务改成等处理函数返回之后再 `finish`——`SessionHandle::finish` 是先到先得，而响应头与上游连接的结束可能落在驱动任务的同一次 poll 里；不加这个等待，407 的失败结论会被驱动任务的 `Completed` 盖掉（实测确实如此，不是理论上的竞争） |
| 终审 D | 4.4（`headers=`）与 6.6（脱敏后的定义）未把自定义 header 的值算作凭据 | `headers=` 的值整体脱敏（连 header 名一起抹掉，`crates/rurge-config/src/redact.rs` 的 `SECRET_PARAMS`），`GET /v1/profiles/current`、`GET /v1/policies/detail` 与 `lineHash` 一并生效。代价与其它凭据一致：只改动 `headers=` 时 `lineHash` 不变（`docs/api/phase2.md` 已注明） |
