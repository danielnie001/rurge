...
- **`ReloadReport.ok` 在重绑监听器失败时也是 `false`，但含义与另外两条失败路径不同**：`run.rs` 的 `reload()` 里，解析失败、构建 `Runtime` 失败这两条路径都在 `d.engine.swap_runtime(next)` 之前返回，运行中的配置保持不变；重绑监听器失败发生在 `swap_runtime` **之后**——`swap_runtime` 一旦构建出新 `Runtime` 就无条件执行，随后才尝试 `rebind_listeners`——所以这条路径的 `ok:false` 意味着新的配置代已经生效，只是监听器归零，直到下一次重载成功为止（下一次重载会无条件重试绑定，与兼容性清单第 809 行「零监听器」退化态一致）。三条路径都返回 `ok:false` 且 `errors` 至少为 1；只有三者都成功才是 `ok:true`。
...
- **`POST /v1/features/{name}` 的 501 文案**（§5.2）：`{"error":"feature `<name>` is not available in this phase"}`（未知功能名仍是 404）。设计文档只写了「501」，没有定这句文案。
- **`/v1/traffic` 的 `startTime` 是 API 启动时刻，不是进程启动时刻**（§5.2）：取 `rurge_api::router` 构造 `Shared` 时的 `SystemTime::now()`。两者在 `rurge run` 里只差几毫秒（API 在监听器之后启动），但语义上以 API 为准；配置里没有 `http-api` 时这个字段根本不存在。
