...
| 第二轮修复波 H | `rurge run` 增加 per-data-dir 实例锁：`run()` 在 `RuntimeArgs::resolve` 之后、`StateStore::open` 之前对 `<data-dir>/rurge.lock` 取 `std::fs::File::try_lock`，`WouldBlock` 退出 1，其它失败记 WARN 后不带检查继续；另补一个单元测试与一个端到端用例（活着的第一个实例 + 第二次启动）。**偏离本计划 Global Constraints**：工作区 `rust-version` 由 1.88 提到 1.89（第 9 行与第 31 行写的都是 1.88），因为 `File::try_lock` 在 1.89 才稳定 | 修复波 A 把崩溃恢复提到绑定之前，只考虑了「上一次运行已经死了」：第一个实例还活着时，同一数据目录上的第二次启动会把它的备份恢复掉并清空，而第一个实例仍报告系统代理开启、重载也不重新应用，流量静默走直连（A 之前这个双启动由先失败的绑定顺手挡住）。用户裁定用标准库的 `File::try_lock` 并接受 MSRV 提到 1.89（本机 rustc 1.89.0，CI 用 `stable`），不引入新依赖 |
| 第二轮修复波 I | `check_user_scope(scope, uid, sudo)`：`sudo` 取自环境变量 `SUDO_UID` 是否存在，`run()` 里算一次并传给 install / uninstall 两条路径；单元测试补 `(User, None, true)` 拒绝、`(System, None, true)` 允许两行；`mod service` 的 CLI 测试从子进程环境里移除 `SUDO_UID` | 修复波 E 的 `check_user_scope` 只看 `uid()`，而 `uid()` 只在 macOS 上探测，`sudo rurge service install --user` 在 Linux 上仍会被接受并把 unit 写进 root 的 `~/.config/systemd/user`。不改为在 Linux 上探测 uid：容器里以 root 跑测试套件很常见 |
